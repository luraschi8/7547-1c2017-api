...
public class CiudadDAOImpl extends DAOImpl implements ICiudadDAO {
	...
	public List<CiudadModelo> listar() {
		...
		List<CiudadModelo> lista = (List<CiudadModelo>)session.createSQLQuery("select * from "  + CiudadModelo.class.getName() + " where borrado = 0").addEntity(CiudadModelo.class).list();
		session.close();
		return lista;
	}
